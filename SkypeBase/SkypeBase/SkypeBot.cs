        ...
        void SkypeMessageEvent(ChatMessage Message, TChatMessageStatus status)
        {
            ...
            if (Message.Status == TChatMessageStatus.cmsSending || Message.Status == TChatMessageStatus.cmsReceived && LastChatMessageId < Message.Id || Message.Status == TChatMessageStatus.cmsRead && LastChatMessageId < Message.Id)
            {
                ...
                if (Message.Body.IndexOf(CommandPrefix) == 0)
                {
                    ...
                    if (banned.Keys.Contains(MessageSender))
                    {
                        Message.Chat.SendMessage("I'm sorry " + MessageSenderDisplayName + ", but you're banned from using " + BotName + ".");
                    }
                    else
                    {
                        ...
                        {
                            if (args[0].ToLower().Equals("login"))
                            {
                                if (admins.Contains(MessageSender))
                                {
                                    onlineAdmins.Add(MessageSender);
                                    Message.Chat.SendMessage(MessageSender + ", you have logged into the admin control panel.");
                                }
                                else
                                {
                                    Message.Chat.SendMessage("You're not an admin!");
                                }
                            }
                            if (onlineAdmins.Contains(MessageSender))
                            {
                                if (args[0].ToLower().Equals("logout"))
                                {
                                    Message.Chat.SendMessage(MessageSender + ", you have logged out of the admin control panel.");
                                }
                                else if (args[0].ToLower().Equals("ban"))
                                {
                                    if (args[2] != "" || args[2] != " ")
                                    {
                                        banned.Add(args[1], args[2]);
                                    }
                                    else
                                    {
                                        banned.Add(args[1], "Banned!");
                                    }
                                    Message.Chat.SendMessage(args[1] + " is now banned!");
                                }
                            }
                            else
                            {
                                Message.Chat.SendMessage("Please login to use the admin commands!");
                            }
                        }
                        else
                        {
